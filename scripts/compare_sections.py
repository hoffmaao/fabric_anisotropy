...
HDR = ('%-15s%5s%9s%9s%8s%8s%7s%6s%6s%6s'
       % ('frame', 'n', 'median', 'mean', 'p10', 'p90', 'max',
          '>1e-3', '>5e-3', 'dth0'))
ROW = '%-15s%5d%+9.5f%+9.5f%+8.4f%+8.4f%7.4f%6.3f%6.3f%s'
...
def report(name, d, dth):
    """One table row for a delta sample; returns nothing if empty."""
    if d.size == 0:
        print('%-15s%5d  no block with a usable deep contrast in both'
              % (name, 0))
        return
    print(ROW % (name, d.size, np.median(d), d.mean(),
...
def main():
    ...
    if not pooled:
        raise SystemExit('no frame comparable between %s and %s'
                         % (before, after))
    print(HDR)
    report('POOLED', np.concatenate(pooled), np.nan)
