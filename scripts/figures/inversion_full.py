...
t(51, 87.4, r'$\dfrac{d\,\Delta\phi}{dz} \;\propto\; '
  r'\lambda_x(z)-\lambda_y(z)$', 14)
t(78, 87.4, r'$\Delta t = \dfrac{\Delta\phi}{2\pi f_c},\quad '
  r'g \equiv \dfrac{\Delta S^{2}}{S_{iso}}$', 13)
t(50, 83.4, 'one channel pair, wrapped, and a fringe rate that gives '
...
t(56, 68.2, 'every observable is QUADRATIC in $S$, so the sweep is a '
  'fixed trig combination\nof 16 moments per range bin - the images '
  'are never rotated', 9.4, MUTED)
...
t(56, 59.8, r'$\hat{C}(\psi,z) \;=\; \gamma\,'
  r'\dfrac{\frac{1-\mu^{2}}{2} + \frac{1+\mu^{2}}{2}\cos\delta '
  r'+ i\,\mu\sin\delta}'
  r'{1-\frac{1-\mu^{2}}{2}\left(1-\cos\delta\right)} \;+\; P(\psi)$', 14)
t(56, 53.4, r'$P(\psi) = (a_1+i\,a_2)\sin 2\psi + a_3\sin^{2}2\psi$', 12.4)
t(56, 50.2, 'the antenna pedestal: instrument leakage carried as a term '
  'to be SOLVED,\n'
...
t(56, 38.6, 'no unwrapping anywhere: the fit sees the fringe RATE and '
  'the azimuthal SHAPE',
  9.4, RED)
...
t(56, 32.0, r'$\left(\Phi^{T}\tilde{W}\Phi+\Lambda\right)\hat{x} = '
  r'\Phi^{T}\tilde{W}\tilde{d},'
  r'\qquad x=(\gamma,a_1,a_2,a_3)^{T},\qquad '
  r'\Lambda=\mathrm{diag}(0,\tau,\tau,\tau)$', 12.4)
t(56, 27.4, r'$J(\theta_0,\delta_0,\delta^{\prime}) = '
  r'\tilde{d}^{T}\tilde{W}\tilde{d}'
...
t(73, 14.6, 'DIRECTION  (what one pair cannot give)', 9.6, MUTED,
  weight='bold')
t(73, 10.4, r'$\hat{\theta}_0$', 15.5)
t(73, 6.8, 'recovered from the azimuthal shape,\nnot from a cross-pol '
  'minimum', 9.0, MUTED)
