...
C_ICE = 3e8 / 1.78 / 2
NB_Z = 25; BLK = 125
ZONES = {"west fan  x 200-1800": (200, 1825), "centre    x 3500-5500": (3500, 5500), "east      x 7000-9500": (7000, 9500)}
...
with h5py.File(fn, "r") as f:
    ...
    nz = r1 // NB_Z
    zc = z[NB_Z//2::NB_Z][:nz]
    print("%-22s %-10s %8s %8s %8s   %s" % ("zone", "band", "|C|plain", "|C|ramp", "|C|1look", "ramp cyc/blk (median |k|)"))
    for zn, (x0, x1) in ZONES.items():
        nb = (x1 - x0) // BLK; x1 = x0 + nb * BLK
        ...
        Pb = P.reshape(nz, nb, BLK); Ab = A.reshape(nz, nb, BLK).sum(2); Bb = B.reshape(nz, nb, BLK).sum(2)
        c_plain = np.abs(Pb.sum(2)) / np.sqrt(Ab * Bb + 1e-30)
        # local ramp per block: mean adjacent-trace phasor product (smoothed over
        # 5 depth cells for stability), then demodulate
        ph = Pb / (np.abs(Pb) + 1e-30)
        adj = (ph[:, :, 1:] * np.conj(ph[:, :, :-1])).sum(2)
        k = np.angle(adj)                                   # rad per trace
        ks = np.copy(k)
        for i in range(nz):
            lo = max(0, i-2); hi = min(nz, i+3)
        ...
        for lo_, hi_ in BANDS:
            m = (zc >= lo_) & (zc < hi_)
            print("%-22s %4d-%4d %8.3f %8.3f %8.3f   %.2f" % (zn, lo_, hi_, np.nanmedian(c_plain[m]), np.nanmedian(c_ramp[m]), np.nanmedian(c_1[m]), np.nanmedian(np.abs(ks[m])) * BLK / (2*np.pi)))
