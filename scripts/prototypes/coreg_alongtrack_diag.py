#!/usr/bin/env python3
"""Is the margin coherence loss ALONG-TRACK phase variation inside the
multilook?
...
NB_Z = 25
WINS = [1, 3, 7, 15, 51]
ZONES = {"west fan  x 200-1800": (200, 1800),
...
with h5py.File(fn, "r") as f:
    ...
    z = T * C_ICE
    la = np.array(f["Latitude"]).ravel()
    lo = np.array(f["Longitude"]).ravel()
    dph = np.radians(np.diff(la))
    dlo = np.radians(np.diff(lo))
    aa = (np.sin(dph/2)**2
          + np.cos(np.radians(la[:-1]))*np.cos(np.radians(la[1:]))
          * np.sin(dlo/2)**2)
    step = 2*6371e3*np.arcsin(np.sqrt(aa))
    print("trace spacing: median %.2f m, p10 %.2f, p90 %.2f; frame %.1f km"
          % (np.median(step), np.percentile(step, 10),
             np.percentile(step, 90), step.sum()/1e3))
    r1 = int(np.searchsorted(z, 1850))
    zc = z[NB_Z//2::NB_Z][: r1 // NB_Z]
    print("%-22s %-10s " % ("zone", "band")
          + " ".join("w=%2d" % w for w in WINS)
          + "   dphi/dx cyc/100tr  |C|_1look(0.5m)")
    for zn, (x0, x1) in ZONES.items():
        ...
        for lo_, hi_ in BANDS:
            ...
            for w in WINS:
                nx = P.shape[1] // w
                Pw = P[:, :nx*w].reshape(nz, nx, w).sum(2)
                Aw = A[:, :nx*w].reshape(nz, nx, w).sum(2)
                Bw = B[:, :nx*w].reshape(nz, nx, w).sum(2)
                c = np.abs(Pw) / np.sqrt(Aw * Bw + 1e-30)
                row += "%5.2f" % np.nanmedian(c[m])
            # circular-mean along-track gradient magnitude in the band
            g = (np.abs(np.angle(np.mean(np.exp(1j*dphx[m]), axis=1)))
                 / (2*np.pi) * 100)
            snr_proxy = np.nanmedian(A[m]) / np.nanmedian(A[(zc >= 1700)])
            row += ("   %6.2f            (HH power / deep-noise %.1f dB)"
                    % (np.nanmedian(g), 10*np.log10(max(snr_proxy, 1e-9))))
            print(row)
